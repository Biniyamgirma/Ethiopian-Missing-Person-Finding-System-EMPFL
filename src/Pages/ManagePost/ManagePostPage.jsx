        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label htmlFor="townId">{t('yourTownId ')}</Label>
              <Input 
                id="townId" 
                name="townId" 
                value={postData.townId || t('townIDnotavailable')} 
                disabled 
                readOnly 
              />
          ...
          <div className="space-y-1">
            <Label htmlFor="subCity">{t('yourSubCity')}</Label>
            <Input 
              id="subCity" 
              name="subCity" 
          ...
          <div className="space-y-1"><Label htmlFor="description">{t('postDescription')}</Label><Textarea id="description" name="description" value={postData.description} onChange={handleInputChange} placeholder={t('postDescriptionPlaceholder')} rows={4} required /></div>
          
          <h3 className="text-lg font-semibold  text-gray-700 dark:text-gray-300 border-t border-gray-200 dark:border-gray-600 mt-4 pt-4">{t('personDetailsSection')}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1">
              ...
              <Select name="personStatus" onValueChange={(v) => handleSelectChange('personStatus', v)} value={postData.personStatus} required>
                <SelectTrigger>
                  <SelectValue placeholder={t('selectPersonStatusPlaceholder')} />
                </SelectTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1"><Label htmlFor="officerId">{t('policeOfficerId')}</Label><Input id="officerId" name="officerId" value={postData.officerId } disabled /></div>
            <div className="space-y-1"><Label htmlFor="stationId">{t('policeStationId')}</Label><Input id="stationId" name="stationId" value={postData.stationId } disabled /></div>
            <div className="space-y-1"><Label htmlFor="status">{t('postStatus')}</Label><Select name="status" onValueChange={(v) => handleSelectChange('status', v)} value={postData.status} required><SelectTrigger><SelectValue placeholder={t('selectStatusPlaceholder')} /></SelectTrigger><SelectContent>{postStatuses.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
          </div>

          <div className="space-y-2"><Label htmlFor="image" className="text-base">{t('uploadImage')}</Label><Input id="image" name="image" type="file" onChange={handleFileChange} accept="image/*" className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"/>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl md:max-w-4xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="p-6 border-b dark:border-gray-700">
          <DialogTitle className="text-xl font-semibold text-gray-800 dark:text-gray-100">{t('editPostDialog')} - ID: {postToEdit.id}</DialogTitle>
          <DialogDescription className="text-gray-500 dark:text-gray-400">{t('editPostDialogDescription')}</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {!isLoadingSpecificTownDetails && !specificTownDetails && !specificTownDetailsError && editedData?.townId && (
                <p className="text-xs text-gray-500 dark:text-gray-400">{t('Nospecificdetailsfoundforthistownoranerroroccurred.')}</p>
              )}
              {!editedData?.townId && !isLoadingSpecificTownDetails && (
                <p className="text-xs text-gray-500 dark:text-gray-400">{t('Selectatowntoviewitsspecificdetails')}</p>
              )}
            </div>

            {/* Post Distribution Status Section */}
            <div>
              <h4 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">{t('PostDistributionStatus')}</h4>
              {isLoadingChecks ? (
        ...
        <DialogFooter className="p-6 border-t dark:border-gray-700 mt-auto">
          ...
          <Button type="submit" form="editPostForm">{t('saveChanges')}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
